...
void Camera::update(const InputMap& input, float delta_time, float aspect)
{
    ...
    {
        glm::vec3 move = forward * input.axis(camera_actions::move_forward)
                       + right * input.axis(camera_actions::move_right)
                       + world_up * input.axis(camera_actions::move_up);

        float speed = move_speed_;
        if (input.held(camera_actions::sprint))
        {
