...
void Ui::begin_frame()
{
    ...
    // layout tree that referenced them is rebuilt. Its BLOCKS survive — a UI's per-frame string
    // volume is roughly constant, so after a few frames this touches the allocator not at all.
    arena_.clear();
    text_capture_ = false;   // re-asserted each frame by whichever field is focused
    clipboard_write_ = {};   // a copy is an EVENT: one frame, then gone
    deferred_.clear();       // ...as is a deferred surface: declared and emitted within one frame
...
{
    // Kept for the many call sites that already build a std::string. `own(string_view)` and
    // `format()` are the cheaper paths — this one has already paid for a temporary by the time it
    // is called, which is exactly the cost `format()` exists to remove.
    return arena_.own(s);
}
...
Element Ui::button(std::string_view name)
{
    ...
    e.color(motion_.animate_color(h, target, theme_.hover))
     ...
     .fit()
     .pad(8)
     .focusable();   // a button is the canonical focus target; it already draws a focus ring
    return e;
}
...
Panel& Panel::handles(ui_action a) { actions_ |= action_bit(a); return *this; }
Panel& Panel::modal() { modal_ = true; return *this; }

bool Panel::took(ui_action a) const { return ui_.interaction_state().took(id_.hash, a); }
...
Panel& Panel::content(const std::function<void(Ui&)>& fn)
{
    ...
    // Resolve the drag BEFORE authoring, so this frame's tree is built at this frame's rect. The
    // header the clamp keeps on screen is this panel's title bar, whose height comes from the THEME
    // — so it is filled in here rather than left to every author to restate (and get wrong).
    panel_limits limits = limits_;
    limits.header.height = static_cast<float>(bar_h);

    panel_state& st = ui_.panels_.panel(id_.hash, initial_);
    update_panel(st, ia, handles_, limits, ia.screen);
    ...
    outer.color(th.panel)
         ...
         .layer(ui_layer::panels)   // above ordinary content, and hit-tests above it
         .z(z)
         .column()
         .gap(0)
         .pad(0);
    if (actions_ != 0) outer.handles_mask(actions_);
    if (modal_) outer.modal();
    ...
    outer.content([&](Ui& u) {
        ...
        // --- Resize grip. Floating, so it sits over the panel's bottom-right corner without taking
        // part in the column flow; LOCAL, so that corner is expressed in the panel's OWN
        // coordinates. It was a root-space SIBLING of the panel, positioned by re-deriving the
        // panel's screen rect — which meant one panel was two surfaces, and moving it wrote two
        // placements that had to agree. In surface coordinates the grip simply rides its panel, and
        // the number it needs is the panel's size, which it already knows.
        //
        // Last in the content, so painter order still puts it over the body.
        Element grip(u, resize_id_);
        grip.color(ia.is_active(handles_.resize)
                       ? th.accent
                       : (ia.is_hot(handles_.resize) ? th.stroke_hi : th.stroke))
            .radius(4)
            .shape(string::shape::ROUNDED_RECTANGLE)
            .fixed(grip_size, grip_size)
            .floating(px(r.width - grip_size - 2.0f), px(r.height - grip_size - 2.0f))
            .local();
    });
...
Element& Element::image(const ::string::image_run& run)
{
    image_ = run;
    has_image_ = true;
    return *this;
...
Element& Element::layer(ui_layer l) { element_.layer = l; return *this; }
...
Element& Element::focusable() { element_.focusable = true; return *this; }
Element& Element::handles(ui_action a) { element_.actions |= action_bit(a); return *this; }
Element& Element::handles_mask(std::uint32_t mask) { element_.actions |= mask; return *this; }
Element& Element::scope() { element_.scope = true; return *this; }
Element& Element::modal() { element_.modal = true; return *this; }

Element& Element::on_click(std::function<void()> fn)
{
    // Registering an activation handler IS a declaration of focusability: an element that can be
    // activated by a click is exactly the thing the gamepad's south button should be able to reach.
    // Saying it here means the common path never has to say it twice, and never gets it half-right.
...
{
    emitted_ = true;
    // An element carries at most one of these. Text wins if both were set — an image with a label
    // is two elements, not one, and silently dropping the text would be worse than the assert-free
    // precedence documented here.
    if (!text_.empty())
        ui_.builder_.add_text(element_, text_, font_px_);
    else if (has_image_)
        ui_.builder_.add_image(element_, image_);
    else
...
void Ui::defer(ui_layer layer, std::function<void(Ui&)> body)
{
    if (body) deferred_.push_back(DeferredSurface{ layer, std::move(body) });
}

bool Ui::emit_next_deferred()
{
    if (deferred_.empty()) return false;

    // Lowest layer first, so a deferred body that declares a HIGHER-layer surface of its own (a
    // submenu over a menu) is still emitted after the thing it anchors to. Linear scan because the
    // count is one or two: a sort would cost more than it saves and would need to be stable.
    auto it = std::min_element(deferred_.begin(), deferred_.end(),
                               [](const DeferredSurface& a, const DeferredSurface& b) {
                                   return static_cast<int>(a.layer) < static_cast<int>(b.layer);
                               });
    DeferredSurface d = std::move(*it);
    deferred_.erase(it);

    // Its own OUTERMOST container, so the builder lays it out independently against the screen —
    // which is what makes it a surface rather than a subtree of the main one. The host closes it.
    builder_.begin(string::format{});
    d.body(*this);
    return true;
}

bounding_box Ui::resolved_box(std::uint64_t id) const
{
    const layout_node* n = builder_.find(id);
    return n != nullptr ? builder_.screen_box(*n) : bounding_box{};
}
...
void Ui::observe()
{
    ...
    for (const layout_node& n : builder_.nodes())
    {
        ...
        if (it != observed_.end()) it->second = builder_.screen_box(n);
    }
}
          ...
          .content([&](Ui& u3) {
              for (std::size_t i = 0; i < cards.size(); ++i)
              {
                  ...
                  tab.color(active ? th.panel_alt : th.panel)
                     ...
                     .pad(padding{ 8, 8, 2, 2 })
                     .fit()
                     .focusable();   // a tab is a button first; it is a drag handle as well
                  tab.content([&](Ui& u4) {
...
void emit_floating(Ui& u, Workspace& ws, node_index index, std::uint8_t z)
{
    ...
    // The header is the 20px move bar emitted at the top of the rect below.
    panel_limits limits{ 180.0f, static_cast<float>(ws.min_region()), panel_header{ 0, 0, 0, 20 } };
    ...
    outer.content([&](Ui& u2) {
        ...
        // Rides the panel's surface in its own coordinates — see the note on the floating panel's
        // grip in PanelView::content for why chrome must not open a surface of its own.
        Element grip(u2, make_id(grip_name));
        grip.color(ia.is_active(handles.resize) ? th.accent : th.stroke)
            .radius(4)
            .shape(string::shape::ROUNDED_RECTANGLE)
            .fixed(16, 16)
            .floating(px(r.width - 18.0f), px(r.height - 18.0f))
            .local();
    });
}
...
void emit_drop_preview(Ui& u, Workspace& ws)
{
    ...
    u.element()
     ...
     // Above every panel by LAYER now, rather than by out-ranking their z numerically. The old
     // comment said "below the debug surfaces" — a layer that stopped existing when brief 14 moved
     // those onto the panel stack, so the number had been arbitrary ever since.
     .z(1);
}
