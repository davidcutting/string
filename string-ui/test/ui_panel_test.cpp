...
// The rule: the HEADER stays wholly on screen. The body may hang off the bottom (there is nowhere
// else for a tall panel to go), but never the strip you grab to drag it back.
TEST(UiPanelTest, MoveKeepsTheWholeHeaderOnScreen)
{
    panel_limits lim{};
    panel_state st{ { 100, 100, 300, 200 }, true };
    update_panel(st, dragging(MOVE, -5000, 5000), handles(), lim, SCREEN);
    EXPECT_FLOAT_EQ(st.rect.x, 0.0f);                                              // flush left
    EXPECT_FLOAT_EQ(st.rect.y, static_cast<float>(SCREEN.height) - lim.header.height);
}

TEST(UiPanelTest, MoveKeepsTheWholeHeaderOnScreenAtTheFarCorner)
{
    panel_limits lim{};
    panel_state st{ { 100, 100, 300, 200 }, true };
    update_panel(st, dragging(MOVE, 5000, -5000), handles(), lim, SCREEN);
    EXPECT_FLOAT_EQ(st.rect.right(), static_cast<float>(SCREEN.width));            // flush right
...
// A header sitting OUTSIDE its rect — the lens's move strip, which floats above the outline. The
// rect being on screen is not enough; the offsets have to enter the clamp or the strip slides off
// the top and the lens can never be dragged back.
TEST(UiPanelTest, HeaderOutsideTheRectIsWhatGetsClamped)
{
    constexpr panel_header lens{ -2.0f, -14.0f, 4.0f, 12.0f };
    panel_limits lim{ 48.0f, 48.0f, lens };
    panel_state st{ { 100, 100, 300, 200 }, true };

    update_panel(st, dragging(MOVE, 0, -900), handles(), lim, SCREEN);
    EXPECT_FLOAT_EQ(st.rect.y, 14.0f);                     // strip top lands exactly on y = 0
    EXPECT_FLOAT_EQ(st.rect.y + lens.dy, 0.0f);

    update_panel(st, dragging(MOVE, 5000, 0), handles(), lim, SCREEN);
    // The strip is 4px wider than the rect, so the rect stops 2px short of the right edge.
    EXPECT_FLOAT_EQ(st.rect.x + lens.dx + st.rect.width + lens.dwidth,
                    static_cast<float>(SCREEN.width));
}

// Degenerate case: a header wider than the viewport cannot be wholly visible, so the bounds invert.
// It must then SPAN the viewport rather than being flung to one side of it.
TEST(UiPanelTest, HeaderWiderThanTheViewportSpansIt)
{
    panel_state st{ { 0, 0, 4000, 200 }, true };
    update_panel(st, dragging(MOVE, 5000, 0), handles(), {}, dimension{ 800, 600 });
    EXPECT_LE(st.rect.x, 0.0f);
    EXPECT_GE(st.rect.right(), 800.0f);
}
...
TEST(UiPanelTest, WindowEmitsChromeWithStableIdsAndPlacesItsBody)
{
    ...
    const bounding_box outer_box = rig.builder.screen_box(*outer);
    EXPECT_EQ(outer_box.x, 50);
    EXPECT_EQ(outer_box.y, 60);
    EXPECT_EQ(outer_box.dimension.width, 320);
    EXPECT_EQ(outer_box.dimension.height, 240);

    // A floating panel is hosted 1:1 by its own SURFACE, and its rect is that surface's placement —
    // which is what makes dragging it free of layout.
    EXPECT_EQ(rig.builder.surfaces()[outer->surface].placement.x, 50);
    EXPECT_EQ(rig.builder.surfaces()[outer->surface].placement.y, 60);
    EXPECT_EQ(outer->box.x, 0) << "a surface's own root sits at its coordinate origin";
    ...
    const bounding_box grip_box = rig.builder.screen_box(*grip);
    EXPECT_EQ(bar->box.dimension.width, 320);              // title bar spans the panel
    // EXACT, not merely "bottom-ish right": the grip moved from a root-space sibling positioned by
    // re-deriving the panel's screen rect to a local child positioned from the panel's SIZE. Those
    // two formulas must land on the same pixel, and an inequality would not have noticed if they
    // stopped doing so. 50 + 320 - 18 - 2 == 350; 60 + 240 - 18 - 2 == 280.
    EXPECT_EQ(grip_box.x, 350);
    EXPECT_EQ(grip_box.y, 280);
    // The chrome rides the panel's surface rather than opening one of its own.
    EXPECT_EQ(grip->surface, outer->surface);
}
...
TEST(UiPanelTest, WindowRectSurvivesTheImmediateModeRebuild)
{
    ...
    const layout_node* outer = rig.builder.find(make_id("stats").hash);
    ASSERT_NE(outer, nullptr);
    EXPECT_EQ(rig.builder.screen_box(*outer).x, 80);
    EXPECT_EQ(rig.builder.screen_box(*outer).y, 70);
}

// --- 12b M2: dragging a surface does not re-lay-out its contents ---------------------------------
//
// THE milestone, as one assertion. A panel's contents resolve in the panel's own coordinates, so
// moving the panel changes its PLACEMENT and nothing else: every local box is bit-identical across
// the move. Before surfaces, `float_x/float_y` were layout inputs and a 1px drag rewrote the box of
// every node in the subtree — which is also what made a moving surface permanently dirty, and so
// what would have defeated M3's clean-surface skip for exactly the surfaces that move most.
TEST(UiPanelTest, MovingAPanelChangesItsPlacementAndNoBoxWithinIt)
{
    Rig rig;
    rig.state.screen = SCREEN;

    const auto author = [&](float x, float y) {
        rig.ui.begin_frame();
        rig.builder.clear();
        rig.builder.begin(format{ .direction = direction::VERTICAL });
        rig.ui.panel("stats").title("Stats").initial({ x, y, 320, 240 }).content([](Ui& u) {
            u.text("body");
            u.text("more body");
        });
        rig.builder.end(SCREEN);
    };

    author(50, 60);
    const layout_node* first = rig.builder.find(make_id("stats").hash);
    ASSERT_NE(first, nullptr);
    const std::uint16_t surface = first->surface;
    std::vector<bounding_box> before;
    for (const layout_node& n : rig.builder.nodes())
        if (n.surface == surface) before.push_back(n.box);
    ASSERT_GT(before.size(), 3u) << "precondition: the panel has a subtree worth re-laying-out";

    // Same panel, moved. `initial` is ignored after the first frame, so drag the title bar instead.
    rig.state = dragging(make_id("stats.title").hash, 137, -25);
    rig.state.screen = SCREEN;
    author(50, 60);

    const layout_node* moved = rig.builder.find(make_id("stats").hash);
    ASSERT_NE(moved, nullptr);
    const bounding_box moved_box = rig.builder.screen_box(*moved);
    ASSERT_EQ(moved_box.x, 187) << "the panel did not actually move — the test proves nothing";
    ASSERT_EQ(moved_box.y, 35);

    std::vector<bounding_box> after;
    for (const layout_node& n : rig.builder.nodes())
        if (n.surface == moved->surface) after.push_back(n.box);

    ASSERT_EQ(after.size(), before.size()) << "the tree changed shape; this is no longer a move";
    for (std::size_t i = 0; i < before.size(); ++i)
    {
        EXPECT_EQ(after[i].x, before[i].x) << "node " << i << " was re-positioned by a MOVE";
        EXPECT_EQ(after[i].y, before[i].y) << "node " << i << " was re-positioned by a MOVE";
        EXPECT_EQ(after[i].dimension.width, before[i].dimension.width);
        EXPECT_EQ(after[i].dimension.height, before[i].dimension.height);
    }
}

// The other half: a moved surface is still hit where it now IS. Local boxes staying put is only
// correct if the placement is applied on the way back out — get that wrong and the panel draws in
// its new home while taking clicks at its old one.
TEST(UiPanelTest, HitTestingFollowsAMovedSurface)
{
    Rig rig;
    rig.state.screen = SCREEN;
    ...
    author();
    rig.state = dragging(make_id("stats.title").hash, 400, 300);
    rig.state.screen = SCREEN;
    author();

    // 460,370 is inside the moved panel (450..770 x 360..600) and outside where it started.
    const layout_node* hit = hit_test_layered(rig.builder, 460, 370);
    ASSERT_NE(hit, nullptr);
    EXPECT_EQ(hit->element.id.hash, make_id("stats.title").hash);

    // ...and the vacated position is no longer the panel.
    const layout_node* stale = hit_test_layered(rig.builder, 60, 70);
    if (stale != nullptr)
        EXPECT_NE(stale->element.id.hash, make_id("stats.title").hash)
            << "the panel still takes clicks where it used to be";
}
...
TEST(UiPanelTest, TitleBarCaptionDoesNotStealTheDragTarget)
{
    ...
    const layout_node* bar = rig.builder.find(make_id("stats.title").hash);
    ASSERT_NE(bar, nullptr);
    // Hit the middle of the title bar, where the caption is. SCREEN space: the hit test takes raw
    // cursor coordinates, so the bar's placement has to be resolved before we can aim at it.
    const bounding_box bar_box = rig.builder.screen_box(*bar);
    const layout_node* hit = hit_test_layered(
        rig.builder, static_cast<std::uint16_t>(bar_box.x + bar_box.dimension.width / 2),
        static_cast<std::uint16_t>(bar_box.y + bar_box.dimension.height / 2));
    ASSERT_NE(hit, nullptr);
    EXPECT_EQ(hit->element.id.hash, make_id("stats.title").hash);
}
...
// A higher LAYER must still beat a lower one regardless of z — layer is the coarse ordering,
// z orders within it. (Guards the bit-packing of the key.)
TEST(UiPanelTest, AHigherLayerBeatsALowerOneWhateverTheZ)
{
    layout_builder b;
    b.begin(format{ .direction = direction::VERTICAL });
    {
        ...
        main.floating = true;
        main.z = 200;   // huge z, but on the CONTENT layer
        b.add_element(main);
    }
    b.end(dimension{ 400, 400 });
