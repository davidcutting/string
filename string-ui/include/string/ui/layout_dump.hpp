                                             ...
                                             std::string_view label = {})
{
    ...
    {
        char buf[96];
        const bounding_box root = nodes.empty() ? bounding_box{} : builder.screen_box(nodes.front());
    ...
    for (std::size_t i = 0; i < nodes.size(); ++i)
    {
        ...
        // SCREEN space, so a dump reads the same whether a subtree is flowed or hosted by a
        // surface — and so every baseline predating surfaces still compares equal.
        const bounding_box box = builder.screen_box(n);
        std::snprintf(buf, sizeof buf, " box=%d,%d,%ux%u", static_cast<int>(box.x),
                      static_cast<int>(box.y),
                      static_cast<unsigned>(box.dimension.width),
                      static_cast<unsigned>(box.dimension.height));
        out += buf;
        ...
        // Named, not numeric: a dump that says "layer=2" makes the reader go and look it up.
        switch (n.element.layer)
        {
            case ::string::ui_layer::content: out += " layer=-"; break;
            case ::string::ui_layer::panels:  out += " layer=panels"; break;
            case ::string::ui_layer::popups:  out += " layer=popups"; break;
            case ::string::ui_layer::modal:   out += " layer=modal"; break;
        }
