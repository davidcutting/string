...
public:
    ...
    // Fill this element's box with a texture. `source` is an opaque id the RENDERER resolves (see
    // image_source in layout.hpp) — the kit never learns what a texture actually is, which is what
    // keeps it dependency-free. Channel mask, mip and range remap are the inspection controls: a
    // depth target or an SDF is unreadable without a remap, and a packed target is unreadable
    // without a channel mask.
    Element& image(const ::string::image_run& run);
    ...
    // Put this element's subtree on a stacking layer (see ui_layer). Replaces the old `.overlay()`,
    // which could only say "on top" and left four call sites inventing z numbers to say the rest.
    Element& layer(ui_layer l);
    ...
    Element& wheel();
    // Make this element a keyboard/gamepad focus target. See layout.hpp: having an id is NOT the
    // same as being focusable, because drag handles and scroll tracks need ids too.
    //
    // Set automatically by `button()`, by the widget factories, and by `on_click()` — registering an
    // activation handler IS a declaration that the element can be activated, so it should not also
    // have to be said separately. Hand-rolled elements that only poll `clicked()` must say it.
    Element& focusable();
    // Declare that this element HANDLES `a`: an action offered to the UI walks up from the focused
    // element and the first ancestor-or-self declaring it receives it. So a button handles
    // `activate` and the dialog around it handles `cancel`, neither knowing about the other.
    Element& handles(ui_action a);
    // Raw mask form, for a caller that has already accumulated one (see Panel).
    Element& handles_mask(std::uint32_t mask);
    // A focus SCOPE: directional nav is confined to this subtree (see layout.hpp).
    Element& scope();
    // A MODAL scope: also swallows actions it does not handle rather than letting them bubble out
    // to the surface behind. Implies scope().
    Element& modal();
...
public:
    ...
    // Declare that this panel HANDLES `a` — an action unclaimed by anything inside it bubbles up to
    // here. A dialog handling `cancel` is the canonical case: none of its children know about it.
    Panel& handles(ui_action a);
    // MODAL: nav is confined to this panel, and an action it does not handle stops here instead of
    // reaching the surface behind. See layout.hpp.
    Panel& modal();

    // Did this panel receive `a` this frame? Valid after content().
    [[nodiscard]] bool took(ui_action a) const;
...
public:
    ...
    // to a widget member is not).
    //
    // Lowest layer first, so a body may itself declare a higher-layer surface (a submenu) and be
    // picked up by the same loop.
    void defer(ui_layer layer, std::function<void(Ui&)> body);

    // HOST SEAM: opens the next deferred surface as its own outermost container and runs its body,
    // returning false when none are left. The caller must CLOSE it — `builder.end(available,
    // measurer)` — because only the host has the measurer. Call in a loop until it returns false.
    ...
    // A copy or cut asks the HOST to put text on the system clipboard — a request for the same
    // reason capture is one: the kit depends on nothing, so it cannot know what a clipboard is.
    // `text` must outlive authoring, so pass an arena view (`own()`), which is what TextField does.
    void request_clipboard_write(std::string_view text) noexcept { clipboard_write_ = text; }
    [[nodiscard]] std::string_view clipboard_write() const noexcept { return clipboard_write_; }
...
private:
    ...
    {
        ui_layer layer = ui_layer::popups;
        std::function<void(Ui&)> body;
    };
    std::vector<DeferredSurface> deferred_;
    bool text_capture_ = false;
    std::string_view clipboard_write_{};
