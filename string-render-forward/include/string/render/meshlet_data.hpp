...
//
// What lives HERE vs in string-asset: the baked geometry representation (GpuMeshlet, GpuMeshletLod,
// the meshopt build constants) is asset-owned — it's what the cook produces and what any renderer
// consumes, so it lives in <string/asset/geometry.hpp>. Everything below is renderer-owned: the
// per-draw table with its material/transform binding, the per-frame GPU stats, and the inspector
// snapshots. The aliases keep those names unqualified in `sandbox` for the pass code.
using ::string::asset::GpuMeshlet;
using ::string::asset::GpuMeshletLod;
using ::string::asset::kMaxLods;
using ::string::asset::kMeshletConeWeight;
