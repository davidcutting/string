...
// A world-space anchor: a point the UI author projects to screen for a world-anchored
// nameplate / ping marker. Client-side, so nameplate projection works IDENTICALLY whether the
// full GeometryPass camera is present (demo scene) or not (ui-dev scene) — the UI never reaches into
// a renderer; it reads this shared view state instead. Nothing here is a rendering concern: it is
// game data (who is on screen, at what health) plus the projection math to place it.
struct UiAnchor
{
    glm::vec3 world{ 0.0f };
