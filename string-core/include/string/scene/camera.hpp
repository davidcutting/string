...
//
// The actions it reads. Interned constants rather than string literals at the query site: the camera
// asks for these every frame, and an action id is an integer compare where a name was a string hash.
// Exposed so an app can rebind them (or bind them to a gamepad) without restating the spelling.
namespace camera_actions
{
inline constexpr ActionId move_forward = action_id("move_forward");
inline constexpr ActionId move_right   = action_id("move_right");
inline constexpr ActionId move_up      = action_id("move_up");
inline constexpr ActionId sprint       = action_id("sprint");
}  // namespace camera_actions
