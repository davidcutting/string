...
: properties_(properties)
{
    ...
    // Seed the clipboard cache. It is refreshed on SDL_EVENT_CLIPBOARD_UPDATE rather than polled:
    // SDL_GetClipboardText allocates on every call, and the clipboard changes almost never, so
    // reading it per frame would be a syscall and a free() per frame for a constant.
    read_clipboard();
...
void Window::update(entt::dispatcher& dispatcher)
{
    ...
    // Service any copy/cut the UI asked for LAST frame, before new_frame clears per-frame state.
    flush_clipboard_write();
    input_.new_frame();
    ...
    while (SDL_PollEvent(&event))
    {
        switch (event.type)
        {
            ...
            // ...and on regaining focus. The UPDATE event is the primary signal, but it can be missed
            // while this window is unfocused — which is precisely the common flow: copy in another
            // app, alt-tab back, paste. Re-reading here costs one syscall per focus change and
            // removes the dependency on an event we cannot verify arrived.
            ...
            case SDL_EVENT_KEY_UP:
            {
                ...
                if (key != KeyCode::UNKNOWN)
                {
                    ...
                    // AUTO-REPEAT passes through as its own signal rather than as a press: the key
                    // never went up, so no press EDGE exists to detect, and text editing needs one
                    // per repeat. SDL owns the delay and rate, which is right — they are an OS
                    // accessibility setting, not something a UI should invent.
                    if (down && event.key.repeat) input_.set_key_repeat(key);
                }
...
void Window::flush_clipboard_write()
{
    std::string text;
    if (input_.take_clipboard_write(text)) set_clipboard_text(text);
}

void Window::read_clipboard()
{
    // SDL hands back an owned copy; free it rather than leaking one per clipboard change. An empty
    // or failed read is a legitimate state (nothing copied yet), not an error to log.
    char* text = SDL_GetClipboardText();
    input_.set_clipboard_text(text != nullptr ? std::string(text) : std::string{});
    if (text != nullptr) SDL_free(text);
}

void Window::set_clipboard_text(std::string_view text)
{
    // SDL wants a NUL-terminated string and the view may not be one.
