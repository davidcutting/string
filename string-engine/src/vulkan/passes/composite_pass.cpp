...
: device_(device)
{
    ...
    pipeline.pipeline_layout = PipelineLayoutBuilder()
...
void CompositePass::update(const float& delta_time, const uint16_t& current_frame)
{
    (void)delta_time;
    (void)current_frame;
}

void CompositePass::record(CommandRecorder& recorder, const uint16_t& current_frame)
{
    (void)current_frame;
    ...
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
        pipeline.pipeline_layout, 0, 1, &descriptor_set_, 0, nullptr);
