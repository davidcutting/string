...
void Workspace::observe(const layout_builder& b) noexcept
{
    for (std::size_t i = 0; i < nodes_.size(); ++i)
    {
        ...
        nodes_[i].resolved = horizontal ? n->box.dimension.width : n->box.dimension.height;
        nodes_[i].box = panel_rect{ static_cast<float>(n->box.x), static_cast<float>(n->box.y),
                                    static_cast<float>(n->box.dimension.width),
                                    static_cast<float>(n->box.dimension.height) };
    }
}
