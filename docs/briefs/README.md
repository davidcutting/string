...
28. **End-of-Phase-A perf + quantization brief** (not yet written) — absorbs `04f` vertex quantization + probe-GI cost reduction + general perf-regression hunt. Runs after 08, before Phase B (hard deadline: quantized/skinned vertex format must be locked before Phase B skinning — **satisfied in spirit by brief 23 (2026-08-13)**: skinning landed first, but only the NEW skin-stream format was locked; the 48B `Vertex` is untouched and the skin stream never re-cooks, so 04f still re-cooks only `kSecVertices`. 04f inherits one constraint from 23: it may re-quantize vertices but must never reorder or renumber them).
30. `23-skeletal-animation.md` — Skeletal animation: playback + blending on ozz-animation 0.17.0 (**the library choice brief 22 flagged as open is now LOCKED**, user decision 2026-08-13). glTF skin/clip import → cooked skin sections (v4) + sibling `.anim` packs → `string::anim` sampling/blending (engine does sampling + blending ONLY; state machines stay game code) → inline mesh-shader skinning through the brief-03 `load_vertex()` seam (built at last, as its own byte-parity-gated milestone). **LANDED 2026-08-13 — M0-M5 in one pass, every headless gate green (identity-palette parity AE=0, Sponza AE=0 throughout, zero validation errors); NEEDS VISUAL VERIFY (the protocol is in the brief).**
